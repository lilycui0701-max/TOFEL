...
function getApiBaseUrl() {
  ...
  const isBackendPort = port === "3000" || port === "3001";
...
async function analyzeResponse() {
  ...
  if (!backendConnected) {
    setAnalysisMessage(
      "The backend is not connected yet. Start the local server with npm start, then refresh the page."
    );
    return;
  }
  ...
  try {
    ...
    const response = await fetch(`${apiBaseUrl}/api/analyze-speaking`, {
      method: "POST",
      headers: {
    ...
    const triedUrls = getApiBaseCandidates().join(", ");
    const message =
      error instanceof TypeError
        ? `Connection failed. Tried these backend URLs: ${triedUrls}.`
...
async function checkBackendHealth() {
  ...
  for (const candidate of candidates) {
    try {
      ...
      setAnalysisMessage(`Backend connected at ${candidate}. Speak, then analyze your response.`);
      return;
    } catch (error) {
  ...
  setAnalysisMessage(
    `Could not connect to the backend. Tried: ${candidates.join(", ")}.`
  );
}
